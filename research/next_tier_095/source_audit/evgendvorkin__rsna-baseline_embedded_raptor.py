...
def load_refit_head(pt_path, feature_dim, device):
    ...
    head = RaptorClassifier(
        nn.Identity(),
        F_dim=int(feature_dim),
    )
    head_state = {
        name: tensor
...
def infer_probs_two_heads(model, refit_head, xwins, device):
    ...
    def forward_heads():
        ...
        original = torch.sigmoid(
            model.head(features).float()
        )
        refitted = torch.sigmoid(
            refit_head.head(features).float()
        )
        return (
            original[0].cpu().numpy(),
            refitted[0].cpu().numpy(),
        )
...
def find_optional_verified_weight(
    fname,
    expected_sha256,
    root='/kaggle/input',
):
    ...
    for path in sorted(set(hits)):
        ...
        print(
            '[head-refit] ignored hash-mismatched '
            f'optional checkpoint: {path}',
            flush=True,
        )
